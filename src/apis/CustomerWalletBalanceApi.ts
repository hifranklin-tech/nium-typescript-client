 ...
 * no description
 */
export class CustomerWalletBalanceApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async walletBalance(clientHashId: string, customerHashId: string, walletHashId: string, xRequestId?: string, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'clientHashId' is not null or undefined
        if (clientHashId === null || clientHashId === undefined) {
            throw new RequiredError("CustomerWalletBalanceApi", "walletBalance", "clientHashId");
        }


        // verify required parameter 'customerHashId' is not null or undefined
        if (customerHashId === null || customerHashId === undefined) {
            throw new RequiredError("CustomerWalletBalanceApi", "walletBalance", "customerHashId");
        }


        // verify required parameter 'walletHashId' is not null or undefined
        if (walletHashId === null || walletHashId === undefined) {
            throw new RequiredError("CustomerWalletBalanceApi", "walletBalance", "walletHashId");
        }
...
export class CustomerWalletBalanceApiResponseProcessor {
     ...
     */
     public async walletBalanceWithHttpInfo(response: ResponseContext): Promise<HttpInfo<Array<WalletBalanceResponseDTO> >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: Array<WalletBalanceResponseDTO> = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "Array<WalletBalanceResponseDTO>", ""
            ) as Array<WalletBalanceResponseDTO>;
