# AUFullPrivateCompanyCustomerDetailsResponse

Contains customer details for AU, corporate, full kycType and Private Company businessType
...
**businessRegistrationNumber** | **string** | official registration number - must be exactly 9 alphanumeric characters for Private Company | [default to undefined]
...
```typescript
import { AUFullPrivateCompanyCustomerDetailsResponse } from 'nium-client';

const instance: AUFullPrivateCompanyCustomerDetailsResponse = {
    wallets,
    customerHashId,
    referenceId,
    status,
