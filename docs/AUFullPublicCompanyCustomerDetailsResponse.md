# AUFullPublicCompanyCustomerDetailsResponse

Contains customer details for AU, corporate, full kycType and Public Company businessType
...
**businessRegistrationNumber** | **string** | official registration number - must be exactly 9 alphanumeric characters for Public Company | [default to undefined]
...
```typescript
import { AUFullPublicCompanyCustomerDetailsResponse } from 'nium-client';

const instance: AUFullPublicCompanyCustomerDetailsResponse = {
    wallets,
    customerHashId,
    referenceId,
    status,
