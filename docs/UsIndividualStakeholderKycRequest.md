# UsIndividualStakeholderKycRequest

Step 3 — Select Resident status for INDIVIDUAL_STAKEHOLDER. isResident: true  → E_KYC (P0) or MANUAL_KYC (P1). isResident: false → MANUAL_KYC only. 
...
```typescript
import { UsIndividualStakeholderKycRequest } from 'nium-client';

const instance: UsIndividualStakeholderKycRequest = {
    region,
    entityType,
    isResident,
    kycMode,
